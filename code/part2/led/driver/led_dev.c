...
{
    int ret;
    size_t status = ((*((volatile size_t*)gpioe_data))>>10)&0x01;//获取GPIOE12状态
    ret = copy_to_user(buff,&status,sizeof(size_t));  //将内核空间拷贝到用户空间buff
    if(ret < 0)
...
static int __init led_init(void)   
{
    ...
    led0 = device_create(led_class,NULL,led_dev_num,NULL,"led0");//创建一个设备名为led0 /dev/led0
    if(IS_ERR(led0))
    {
